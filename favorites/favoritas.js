    ...
    function renderizar() {
        ...
        if (peliculasFavoritas.length === 0) {
            ...
            peliculasFavoritas.forEach(pelicula => {
                ...
                const peliculaHTML = `
                <div class=" p-4 text-center flex justify-center items-center flex-col border border-black w-full sm:w-1/2 md:w-1/3 lg:w-1/3">
                ...
                <h2 class=" text-white">${pelicula.title}</h2>
                <p class=" text-white">${pelicula.overview}</p>
                <a href="${pelicula.homepage}" class=" text-gray-500 hover:text-blue-700 text-xl font-bold uppercase mt-2 inline-block hover:underline">Más información</a>
                <button class="bg-transparent hover:bg-black text-black font-semibold hover:text-white py-2 px-4 border border-black rounded-full transition duration-300 m-10 heart-button" data-pelicula-id="${pelicula.id}">
